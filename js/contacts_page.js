...
function clearHighlight() {
  for (let i = 0; i < contacts.length; i++) {
    document
      .getElementById(`contact-con-${i}`)
      .classList.remove("contact-highlight");
  }
}
...
window.addEventListener("resize", function () {
  if (
    ...
    document.URL.includes("contacts.html")
  ) {
    document.getElementById("contacts-list-section").classList.remove("d-none");
  }
  if (
...
function showPopupContactContent() {
  document
    .getElementById("contacts-add-con")
    .classList.add("contacts-add-con-show");
}
...
function showPopupExistContact(i) {
  ...
  document.getElementById("popuo-contact-user-icon").innerHTML =
    createPopupExistContactIcon(i);
  ...
  document.getElementById(
    "popup-contact-phone"
  ).value = `${contacts[i]["phone_number"]}`;
  document.getElementById("popup-contact-button-con").innerHTML =
    createPopupExistContactBt(i);
}
