...
async function defineNewTask() {
  ...
  let assignedTo = JSON.stringify(getAssignedToUsers());
  let dueDate = document.getElementById("task-date");
  let taskCategory = JSON.stringify(getTaskCategory());
...
function changeBoardStyle(subButton, cardStroy, formContainer) {
  ...
  document.getElementById("board_detail_card").style =
...
async function changeTask(id, i) {
  ...
  let assignedTo = JSON.stringify(getAssignedToUsersEditTask(i));
