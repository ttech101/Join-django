...
function executeOnScreenWidthChange() {
  ...
  if (
    lastScreenWidth < screenWidthThreshold &&
    currentScreenWidth >= screenWidthThreshold
  ) {
    document.getElementById("summary_gretting").classList.remove("summary_dn");
  } else if (
    lastScreenWidth >= screenWidthThreshold &&
...
function showPopupSlider() {
  ...
  popup.innerHTML = `<div class="summary_fbccco">
...
function loadSummaryCategory(category, task) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    if (element[category] == task) task_counter = task_counter + 1;
  }
  return task_counter;
}
...
function summaryIconChangeHover(icon) {
  if (icon == "pen_icon") {
    document.getElementById("summary_edit_icon").src =
      "../img/pen_icon_withe.svg";
  }
  if (icon == "check_icon") {
    document.getElementById("summary_check_icon").src =
      "../img/check_icon_withe.svg";
  }
}
