...
function filterTaskBoard(task_board) {
  ...
  if (filter.length) {
    ...
    for (let i = 0; i < filter.length; i++) {
      ...
      let priority_img =
        "../img/task-prio-" + element.priority.charAt(0).toLowerCase() + ".svg";
      document.getElementById("board_" + task_board).innerHTML +=
        createBoardTasks(
          element.id,
          element.category,
...
function loadBoardUsers(id, task_user) {
  for (let i = 0; i < task_user.length; i++) {
    ...
    if (i < 3) {
      document.getElementById(task_user_number).innerHTML += createBoardUsers(
        element.color,
        element.name
      );
...
function loadBoardSubtasks(id, subtasks) {
  ...
  if (subtasks.length) {
    ...
    document.getElementById(subtask_number).innerHTML = createBoardSubtasks(
      element_subtask,
      subtasks.length,
      percent
    );
...
async function moveTo(category) {
  for (let i = 0; i < list.length; i++) {
    ...
    if (element.id == draggedElement) {
      list[i]["task_board"] = category;
      let taskUser = JSON.stringify(element.task_user);
      let category_new = JSON.stringify(element.category);
      subtasks = element.subtasks;
      taskPrio = element.priority;
      id = element.id;
      await updateTask(
        element.headline,
        element.text,
...
function loadBoardCard(id) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    if (element.id == id) {
      ...
      document.getElementById("board_detail").innerHTML = createBoradCard(
        id,
        story,
        story_bg,
        headline,
...
function createBordCardUsers(id, users) {
  ...
  if (users.length >= 1) {
    ...
    for (let i = 0; i < users.length; i++) {
      ...
      document.getElementById(`board-card-users${id}`).innerHTML +=
        createBoardCardUsers(element.full_name, element.name, element.color);
    }
  }
}
...
async function toggelSubtaskCompleted(id, i, status) {
  for (let k = 0; k < list.length; k++) {
    ...
    if (element.id == id) {
      if (status == 1) {
        element.subtasks[i].completed = 0;
      } else {
        element.subtasks[i].completed = 1;
      }
      let taskUser = JSON.stringify(element.task_user);
      let category = JSON.stringify(element.category);
      subtasks = element.subtasks;
      taskPrio = priority;
      ...
      createBordCardSubtasks(id, list[k]["subtasks"]);
    }
  }
}
